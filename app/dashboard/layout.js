  ...
  return (
    <div className="flex min-h-screen">
        ...
        <div className="flex flex-col transition-all duration-300 border-t border-[#e0e0e0]">
          {/* Slide-down Sign Out Button */}
            ...
            className={`overflow-hidden transition-all duration-300 px-4 text-sm ${
              showProfileOverlay ? 'max-h-20 py-3 border-t border-[#e0e0e0]' : 'max-h-0 py-0'
            }`}
          >
            <button
            ...
            >
              {sidebarOpen? "Sign out" : <LogOut className="-ml-1.5"/>}
            </button>
          </div>
          {/* Profile Row */}
          <div
      ...
      <main className="flex-1 bg-white">
        <div className="sticky top-0 z-1 flex h-14 items-center gap-4 border-b border-[#e0e0e0] bg-background px-4 sm:px-6">
          <button  onClick={() => setSidebarOpen(!sidebarOpen)} className={sidebarOpen ? "transition-opacity duration-300 ease-in-out opacity-0 pointer-events-none" : "transition-opacity duration-300 ease-in-out opacity-100 cursor-pointer"}>☰</button>
